...
func (d *CustomStorageDevice) EncodeValues(key string, v *url.Values) error {
	...
	if d.FileID != nil && (strings.HasSuffix(*d.FileID, ".qcow2") || strings.HasSuffix(*d.FileID, ".raw")) {
...
func (d CustomStorageDevices) EncodeValues(_ string, v *url.Values) error {
	for s, d := range d {
		...
		if d.FileID == nil || *d.FileID == "" || strings.HasSuffix(*d.FileID, ".qcow2") || strings.HasSuffix(*d.FileID, ".raw") {
